import type { RunData, BacktestMetrics } from "@/lib/api";

export type ChartCache = Record<string, Pick<RunData, "price_series" | "indicator_series" | "trade_markers">>;
...
export function buildTradesCsv(trades: Array<Record<string, string>>): string {
...
export function buildMetricsCsv(metrics: BacktestMetrics): string {
  const header = "metric,value";
  const rows = Object.entries(metrics).map(([k, v]) => `${escapeCsvField(k)},${escapeCsvField(v)}`);
  return [header, ...rows].join("\n");
}

export function cacheFromRun(run: RunData | null, requestedSymbol?: string): ChartCache {
